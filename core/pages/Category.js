...
export default {
  ...
  computed: {
    ...
    productsCounter() {
      return this.$store.state.product.list.items
        ? this.$store.state.product.list.items.length
        : 0;
    },
    productsTotal() {
    ...
    isCategoryEmpty() {
      return (
        !this.$store.state.product.list.items ||
        this.$store.state.product.list.items.length === 0
      );
    },
    category() {
  ...
  preAsyncData({ store, route }) {
    ...
    const currentProductQuery =
      store.getters["category/getCurrentCategoryProductQuery"];
    const sort =
    ...
    store.dispatch("category/setSearchOptions", {
      ...
      excludeFields:
        config.entities.optimize && isServer
          ? config.entities.productList.excludeFields
          : null,
      append: false
    });
  },
  async asyncData({ store, route, context }) {
    ...
    try {
      ...
      await store.dispatch("attribute/list", {
        // load filter attributes for this specific category
      ...
      if (subloaders) {
        ...
        await EventBus.$emitFilter("category-after-load", {
          store: store,
          route: route
        });
      } else {
  ...
  methods: {
    ...
    pullMoreProducts() {
      ...
      let current =
        this.getCurrentCategoryProductQuery.current +
        this.getCurrentCategoryProductQuery.perPage;
      ...
      if (this.getCurrentCategoryProductQuery.current <= this.productsTotal) {
        this.mergeSearchOptions({
          searchProductQuery: buildFilterProductsQuery(
            this.category,
            this.filters.chosen
          )
        });
    ...
    onFilterChanged(filterOption) {
      ...
      if (
        ...
        (toString(filterOption.id) ===
          ...
          filterOption.id ===
            this.filters.chosen[filterOption.attribute_code].id)
      ) {
      ...
      this.$store
        ...
        .then(res => {}); // because already aggregated
    },
      ...
      this.$store
        ...
        .then(category => {
          if (!category) {
            ...
            this.$bus.$emitFilter("category-after-load", {
              store: this.$store,
              route: route
            });
          }
        ...
        .catch(err => {
          if (err.message.indexOf("query returned empty result") > 0) {
            ...
            this.$router.push(
              localizedRoute("/", currentStoreView().storeCode)
            );
          }
        });
    },
